...
class UnitTests(unittest.TestCase):
    def assertDictAlmostEqual(self, actual, expected, places=7):
        """
        Custom method to compare two dictionaries containing lists of floats.
        """
        for key in expected:
            for a, e in zip(actual[key], expected[key]):
                if isinstance(a, list) and isinstance(e, list):
                    for av, ev in zip(a, e):
                        self.assertAlmostEqual(av, ev, places=places)
                else:
